...
SET @idoUUID=COALESCE(@idoUUID,
      (
        SELECT
          ido.UUID
        FROM InventoryDocumentOrder ido 
          INNER JOIN Store s1 ON s1.ID=ido.StoreID
          INNER JOIN Store s2 ON s2.ID=ido.TargetStoreID
        WHERE 1=1
...
SELECT
  CONCAT(
    CASE WHEN (
      SELECT COUNT(*) FROM InventoryDocumentOrderDetail q
    ...
    END,
    CASE WHEN (
      SELECT SUM(Quantity) FROM InventoryDocumentOrderDetail q
      WHERE q.InventoryDocumentOrderUUID=ido.UUID
  ...
  ido.ErpDocumentID
FROM InventoryDocumentOrder IDO 
  LEFT JOIN DocumentStatus    ds ON ds.ID=ido.DocumentStatusID
  LEFT JOIN DocumentType      dt ON dt.ID=ido.DocumentTypeID
  LEFT JOIN Store             s1 ON s1.ID=ido.StoreID
  LEFT JOIN Store             s2 ON s2.ID=ido.TargetStoreID
  LEFT JOIN Warehouse         w1 ON w1.ID=ido.WarehouseID
  LEFT JOIN Warehouse         w2 ON w2.ID=ido.TargetWarehouseID
  LEFT JOIN InventoryDocument id ON id.OriginInventoryDocumentOrderUUID=ido.UUID
...
FROM InventoryDocumentOrderDetail 
WHERE
...
--    idod.Serial
  FROM InventoryDocumentOrderDetail IDOD 
    INNER JOIN InventoryDocumentOrder IDO ON IDO.UUID=IDOD.InventoryDocumentOrderUUID
    LEFT JOIN InventoryOnHand IOH ON IOH.WarehouseID=IDO.TargetWarehouseID
      AND IOH.ItemID=IDOD.ItemID
      AND COALESCE(IOH.ItemCombinationID,0)=COALESCE(IDOD.ItemCombinationID,0)
      AND IOH.StoreID>0
    LEFT JOIN InventoryOnHandDetail IOHD ON IOHD.InventoryOnHandUUID=IOH.UUID
...
--      AND COALESCE(IOHD.CustomsInfo,'')=COALESCE(IDOD.CustomsInfo,'')
    LEFT JOIN InventoryOnHandUnitDetail IOHUD ON IOHUD.InventoryOnHandDetailUUID=IOHD.UUID
  WHERE
